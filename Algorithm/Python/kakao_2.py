...
for i in range(0,len(query)):
  q_list += query[i].replace('and ','').split(',')
  q_list2.append(q_list[i].split(' '))

for j in range(0,len(info)):
  i_list.append(info[j].split(' '))


for k in range(0,len(i_list)):
  for l in range(0,len(i_list[k])):
    
    
    if i_list[k][l] == q_list2[k][l] or i_list[k][l] > q_list2[k][l] :
      print("A")
    else:
      print("B")
